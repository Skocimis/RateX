...
npx hardhat run scrips/utils/fundHardhat.js --network localhost       - funds the first hardhat account
```
...
npx hardhat run scrips/utils/fundTenderly.js --network tenderly       - funds the first wallet account
```
Then, from the RateX folder run `npm start`.
